...
def objective(trial):
    ...
    # window_size = [3]#trial.suggest_int('lags', 2, 4)
    ...
    for i in range(n_layers):
        ...
        if i % 2 == 0:
            # NN_config.append('d')
            NN_neurons.append(trial.suggest_int(f'n_units_{i}', 4, 16, step=4))
        else:
            # NN_config.append('dr')
            NN_neurons.append(trial.suggest_float(f'dropout_{i}', 0.2, 0.4, step=0.1))
    # callbacks = [
    ...
    params = {
        ...
        'x_test': data_test,
        'run': trial.suggest_int('run', 2, 5, step=1),
        'epochs_num': [
            trial.suggest_int('epochs_1', 30, 100, step=10),
            trial.suggest_int('epochs_2', 20, 50, step=10)
        ],
        'learning_rate': [
        ...
        'x_val': data_val,
        'reg_alpha': trial.suggest_float('reg_alpha', 1e-5, 1e-4, log=True),
        'callbacks': None,
        'verbose': False,
        'plot': False
    }
    ...
    try:
        ...
        avg_error = total_error / len(window_size)
...
study = optuna.create_study(
    ...
    pruner=optuna.pruners.MedianPruner(
        ...
        n_warmup_steps=8,         # 在开始剪枝之前的训练步数
        interval_steps=1           # 进行剪枝检查的间隔步数
    ),
    sampler=optuna.samplers.TPESampler(
        seed=42,
        multivariate=True, # 考虑参数间的关系
        n_startup_trials=10 # TPE采样器在随机采样阶段的试验数
        )
)
# 添加回调函数监控训练过程
...
study.optimize(objective, 
               n_trials=30, 
               n_jobs=-1,
               catch=(Exception,), # 捕获异常，防止单次失败影响整体运行
